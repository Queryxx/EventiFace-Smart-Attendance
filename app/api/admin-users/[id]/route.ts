...
export async function PUT(request: Request, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    const existingUser = await sql("SELECT id FROM admins WHERE (username = $1 OR email = $2) AND id != $3", [username, email, params.id])
...
export async function DELETE(request: Request, { params }: { params: { id: string } }) {
  ...
  try {
    await sql("DELETE FROM admins WHERE id = $1", [params.id])
