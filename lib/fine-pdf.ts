...
export function exportFineReceiptPDF(fine: FineReceipt) {
  ...
  doc.text("EventiFace System", margin, margin + 26)
  ...
  doc.setFont("helvetica", "normal")
  doc.text(new Date(fine.date).toLocaleDateString(), pageWidth - margin - 20, margin + 35)
  ...
  doc.setFont("helvetica", "bold")
  doc.text("Student Number:", margin, margin + 62)
  doc.setFont("helvetica", "normal")
  doc.text(fine.student_number, margin + 35, margin + 62)

  // Fine Details
  doc.setFontSize(10)
  doc.setFont("helvetica", "bold")
  doc.text("FINE DETAILS", margin, margin + 72)

  doc.setFontSize(9)
  doc.setFont("helvetica", "bold")
  doc.text("Reason:", margin, margin + 79)
  doc.setFont("helvetica", "normal")
  const reasonLines = doc.splitTextToSize(String(fine.reason), pageWidth - margin * 2 - 35)
  doc.text(reasonLines, margin + 35, margin + 79)

  const reasonHeight = reasonLines.length * 5
  const amountYPos = margin + 79 + reasonHeight + 10

  doc.setFont("helvetica", "bold")
  doc.text("Status:", margin, amountYPos)
  doc.setFont("helvetica", "normal")
  doc.text(fine.status || "Unpaid", margin + 35, amountYPos)
...
export function exportMultipleFinesPDF(fines: FineReceipt[]) {
  ...
  const tableData = fines.map((fine) => [
    ...
    String(new Date(fine.date).toLocaleDateString()),
    String(fine.status || "Unpaid"),
  ])

  autoTable(doc, {
    head: [["Student #", "Student Name", "Reason", "Amount", "Date", "Status"]],
    body: tableData,
    startY: margin + 25,
    margin: margin,
    headStyles: {
  ...
  doc.setFont("helvetica", "bold")
  doc.setFontSize(11)
  doc.text(`Total Fines: ₱ ${totalAmount.toFixed(2)}`, pageWidth - margin - 30, finalY, { align: "right" })
